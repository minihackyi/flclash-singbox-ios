...
class OfficialSubscriptionSyncResult {
  const OfficialSubscriptionSyncResult({
...
class SetupAction extends _$SetupAction {
  ...
  DateTime? startTime;
  num _lastReportUp = 0;
  num _lastReportDown = 0;
  static bool _revokedNotified = false;
  ...
  Future<void> _reportTraffic() async {
    if (!authSession.isAuthenticated) return;
    final traffic = ref.read(totalTrafficProvider);
    final up = traffic.up;
    final down = traffic.down;
    final deltaUp = up - _lastReportUp;
    final deltaDown = down - _lastReportDown;
    _lastReportUp = up;
    _lastReportDown = down;
    if (deltaUp <= 0 && deltaDown <= 0) {
      await _sendHeartbeat(online: true);
      return;
    }
    final result = await authSession.reportTraffic(up: deltaUp, down: deltaDown);
    if (result.status == 'expired' || result.status == 'exhausted') {
      await _handleSubscriptionRevoked(result.status, result.message);
    }
  }

  Future<void> _sendHeartbeat({bool online = true}) async {
    if (!authSession.isAuthenticated) return;
    final result = await authSession.heartbeat(online: online);
    if (result.status == 'expired' || result.status == 'exhausted') {
      await _handleSubscriptionRevoked(result.status, result.message);
    }
  }

  Future<void> _handleSubscriptionRevoked(String status, String? message) async {
    final targetUrl = authSession.lastSubLink;
    if (targetUrl != null && targetUrl.isNotEmpty) {
      final targets = ref
          .read(profilesProvider)
          .where((p) => p.url == targetUrl)
          .toList();
    ...
    // 防止删除官方订阅后自动回退到旧的本地订阅
    if (ref.read(isStartProvider)) {
      await updateStatus(false);
    }
    if (!_revokedNotified) {
      _revokedNotified = true;
      final text =
          message ??
          (status == 'expired'
              ? '订阅已到期，请联系管理员续期'
...
class ProfilesAction extends _$ProfilesAction {
  ...
  Future<void> addProfileFormURL(String url) async {
    ...
    for (int attempt = 0; attempt < 2; attempt++) {
      if (attempt == 0) {
        ...
        // 第二次正式尝试（有 loading + 错误提示）
        profile = await globalState.loadingRun(
          tag: LoadingTag.profiles,
          () async {
  ...
  Future<Profile?> _fetchSubscriptionProfile(String url) async {
  ...
  /// Check the official account and keep its subscription in sync.
  ///
  /// The check is intentionally conservative with existing profiles: it reuses
  /// the current official profile instead of creating duplicates. A new profile
  /// is only created when the account has no matching subscription locally.
  Future<OfficialSubscriptionSyncResult> syncOfficialSubscription({
    bool refreshUser = true,
  }) async {
    if (!authSession.isAuthenticated) {
      return const OfficialSubscriptionSyncResult(
        success: false,
        message: '请先登录官网账号',
      );
    }

    final oldSubLink = authSession.user?['subLink']?.toString();
    if (refreshUser) {
      final error = await authSession.refreshUser();
      if (error != null) {
        return OfficialSubscriptionSyncResult(success: false, message: error);
      }
    }

    final user = authSession.user;
    final subLink = user?['subLink']?.toString();
    final plan = user?['plan']?.toString();
    final status = user?['status']?.toString();
    final revokeReason = user?['revokeReason']?.toString();
    if (status == 'expired' ||
        status == 'exhausted' ||
        status == 'blacklisted' ||
        (revokeReason != null && revokeReason.isNotEmpty)) {
      final reason = revokeReason ?? status ?? 'expired';
      final revokedMessage = reason == 'exhausted'
          ? '流量已用完，请联系管理员补充流量'
          : reason == 'blacklisted'
          ? '账号已被加入黑名单，请联系管理员'
          : '订阅已到期，请联系管理员续期';
      final revokedLink = (oldSubLink != null && oldSubLink.isNotEmpty)
          ? oldSubLink
          : (subLink != null && subLink.isNotEmpty
              ? subLink
              : authSession.lastSubLink);
      if (revokedLink != null && revokedLink.isNotEmpty) {
        final targets = ref
            .read(profilesProvider)
            .where((p) => p.url == revokedLink)
            .toList();
        for (final profile in targets) {
      ...
      return OfficialSubscriptionSyncResult(
        success: true,
        revoked: true,
        message: revokedMessage,
      );
    }
    if (subLink == null || !subLink.startsWith('http') || plan == '待开通') {
      return const OfficialSubscriptionSyncResult(
        success: false,
        message: '当前账号暂无可用订阅',
      );
    ...
    for (final item in profiles) {
      if (item.url == subLink ||
          (oldSubLink != null && item.url == oldSubLink)) {
        profile = item;
        break;
      }
    }
    ...
    if (profile == null) {
      final newProfile = await _fetchSubscriptionProfile(subLink);
      if (newProfile == null) {
        return const OfficialSubscriptionSyncResult(
          success: false,
          message: '订阅下载失败，请稍后重试',
        );
      }
    ...
    final linkChanged = profile.url != subLink;
    final candidate = linkChanged ? profile.copyWith(url: subLink) : profile;
    Uint8List before;
    try {
      final file = await candidate.file;
      before = await file.readAsBytes();
    } catch (_) {
      before = Uint8List.fromList([]);
    }

    // 小黑机场订阅首次更新必失败，10秒内重试第二次才成功。
    Object? lastError;
    for (var attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        Uint8List after;
        try {
          final file = await updatedProfile?.file;
          after = await file?.readAsBytes() ?? Uint8List.fromList([]);
        } catch (_) {
          after = Uint8List.fromList([]);
        }
        final contentChanged = !listEquals(before, after);
        final subscriptionInfoChanged =
            updatedProfile?.subscriptionInfo != profile.subscriptionInfo;
        return OfficialSubscriptionSyncResult(
          success: true,
          changed: linkChanged || contentChanged || subscriptionInfoChanged,
        );
      } catch (error) {
        lastError = error;
        if (attempt == 0) {
    ...
    return OfficialSubscriptionSyncResult(
      success: false,
      message: lastError?.toString() ?? '订阅更新失败',
    );
  }

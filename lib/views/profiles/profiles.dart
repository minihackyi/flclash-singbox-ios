...
class ProfileItem extends StatelessWidget {
  ...
  Future<void> _verifyAdminThenOpen(
    ...
    final result = await authSession.verifyAdmin(username, password);
    if (!context.mounted) return;
    if (result.success) {
      open();
    } else {
      context.showNotifier(result.error ?? '管理员账号或密码错误');
    }
  }
                ...
                child: isUpdating
                    ...
                    : CommonPopupBox(
                        ...
                        targetBuilder: (open) {
                          return IconButton(
                            onPressed: () {
                              _verifyAdminThenOpen(context, open);
                            },
